...
function toInheritValues(invoke, id) {
	const ret = { ...invoke };
...
function rewriteInheritValues(inherit) {
	...
	// if (tmp.duration) {
	//   tmp.duration = tmp.duration * 0.6;
	// }

	// if (tmp.invokes) {
	//   for (const invoke of tmp.invokes) {
	//     if (invoke.duration) {
	...
	for (const effect of effects) {
		if (tmp[effect] && inheritMap[effect]) {
			const map = inheritMap[effect];
			const value = inherit[effect];
			let newValue = map[value];
			if (newValue == null) {
				console.error(`Unknown ${effect} value ${value} in ${JSON.stringify(tmp)}`);
			}
			if (tmp.id == 100991) {
				//빛나라☆토마코마이
				newValue = 0.07;
	...
	if (tmp.invokes) {
		variant.invokes = [];
		for (let i in tmp.invokes) {
			variant.invokes[i] = toInheritValues(tmp.invokes[i], tmp.id);
		}
		delete tmp.invokes;
	}
	...
	methods: {
		...
		checkCondition(skill, cond, value) {
			...
			switch (cond) {
				...
				case "base_stamina":
					return () => thiz.umaStatus.stamina >= value;
				case "base_power":
